...
**The "nuclear" complete fix** — an exclusive-fullscreen-only-when-fullscreen hybrid — is the *only categorical cure* but is high-risk: it requires reparenting the mpv child to a top-level swapchain (the exact move HANDOFF landmine #10 says made video disappear), fights the focus-loss-auto-minimize semantics that *define* the problem, and its HDR survival through that reparent is unverified.
...
| 7 | **Exclusive-fullscreen hybrid** (true `SetFullscreenState(TRUE)` only when fullscreen + overlay hidden) | **Yes** — DWM disabled for that output (only categorical cure) | Theoretically yes; **unverified** through mpv's render path | **Very high** / very high — reparent = HANDOFF landmine #10 ("video disappeared"); auto-minimize on focus loss happens *exactly when* the fix is needed; black-flash mode switches on every overlay interaction | Does mpv's gpu-next HDR survive an exclusive reparented swapchain at all? |
...
- **Reparenting the mpv child to a top-level window (`SetParent(NULL)` + `WS_POPUP`) — known failure.** HANDOFF landmine #10: libmpv's render context didn't survive the reparent, video disappeared. Any exclusive-FS hybrid must solve this first.
