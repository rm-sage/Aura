...
// ---------------------------------------------------------------------------
// Hover-seek thumbnails — headless 2nd-libmpv frame engine.
//
// The extractor's engine lives in `mpv2::thumb`: a self-owned `vo=null`
// libmpv instance created via direct `libmpv-2.dll` FFI on a dedicated
// worker thread. Per request it (re)loadfiles when the URL changes, seeks
// absolute+exact, confirms the seek, frame-steps a fresh decode, then
// `screenshot-to-file` (video, no OSD/subs) → base64 data URL. The old
// implementation drove a `tauri-plugin-libmpv` named "thumb" instance;
// porting it to FFI removed the plugin from the thumbnail path (the Phase-7
// blocker). EVERYTHING fails gracefully to `Ok(None)` so the scrubber falls
// back to the timestamp-only tooltip — it can never break the main player.
// Output is pre-scaled to 320 px wide so each data URL is a few KB.
// ---------------------------------------------------------------------------
...
///
/// Defined here (platform-agnostic) so the `extract_thumbnail` command and
/// its serialisation exist on every target; the Windows-only `mpv2::thumb`
/// engine constructs it.
...
pub async fn extract_thumbnail(
    ...
    // The headless engine (`mpv2::thumb`) owns a dedicated worker thread; do
    // the blocking submit-and-wait off the async runtime. Windows-only — the
    // mpv2 FFI engine is Windows-gated; on other targets thumbnails are
    // simply unavailable and the scrubber uses the timestamp tooltip.
    #[cfg(target_os = "windows")]
    {
        tauri::async_runtime::spawn_blocking(move || {
            crate::mpv2::thumb::extract(url, at_seconds)
        })
        .await
        .map_err(|e| e.to_string())?
    }
